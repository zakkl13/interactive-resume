// PDF resume, built with Typst. (Currently 2 pages with full content; trimming
// to one page is deferred.)
//
...
#resume-lead[Tech lead for Fire TV Catalog Enrichment, previously tech lead for Fire TV Partner Integrations. Promoted to Sr. Software Eng (Q4 2022).]
...
#resume-item[
  - Built an LLM genre classifier that replaced unreliable provider data, embedded in the catalog's stream-processing pipeline at *\~\$0.001 per entity*; engineered an eligibility filter scoping inference to the \~5% of incoming updates to keep long-tail content off the latency-sensitive path.
  ...
  - Tech lead on subsequent enrichment features (location extraction from linear-station metadata, image metadata extraction) shipped by mid-level engineers; core contributor to the catalog org's shared Bedrock integration library.
]

#resume-project("3P Continue Watching on Fire TV", stack: "Mobile-Cloud Architecture, Rust, Android, Java Services")
#resume-item[
  ...
  - Built and operated the cloud ingestion backend handling sensitive customer and partner data, peaking at up to  *\~200K TPS*.
  - Owned the privacy/consent surface end-to-end. Opt-in capture, consent handling, and implemented copy experimentation that drove *opt-in from 30% -> 75%* over a few months.
]
...
#resume-lead[Led cloud modernization initiatives for Global Top 10 insurance client. Promoted to Sr. Consultant (Q3 2019).]
